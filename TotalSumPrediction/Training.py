...
def main():
    ...
    # Logging
    #
    num_classes = (MAX_NUMBER - 1)*SEQ_LEN
    model_list = [BasicRNN_Model(num_classes), LSTM_Model(num_classes), GRU_Model(num_classes)]
...
def prepare_data(dataset):
    ...
    dataset = dataset.map(lambda x, target: (tf.squeeze(x), target))

    dataset = dataset.map(lambda x, target: (x, tf.one_hot(target, depth=(MAX_NUMBER - 1)*SEQ_LEN, axis=-1)))
    dataset = dataset.map(lambda x, target: (x, tf.squeeze(target)))
